...
Тут все понятно - какой то список или массив объектов, соотв. провайдерам, все реализуют какой то метод, запрашивающий состояние погоды и возвращающий ее в виде удобном для хранения в базе. Вопрос стоит только в создании такого массива или списка. Заранее мы не знаем сколько у нас провайдеров. Тут два очевидных решения - жестко в коде прописывать создание объектов (тогда придется править это место каждый раз при добавлении нового провайдера) или составлять этот список в рантайме (использовать рефлексию). Я решил через рефлексию - прокачать скилы + это позволяет реализовать бесшовную интеграцию (именно позволяет, я полностью без швов не реализовал, ниже подробнее)
